...
class ApiLlm:
    """Real LLM API client supporting Messages API, OpenAI, and local CLI providers.

    Example:
        llm = ApiLlm.from_env()
        llm = ApiLlm.messages_api("sk-ant-...", "claude-sonnet-4-20250514")
        llm = ApiLlm.openai("sk-...", "gpt-4o")
        llm = ApiLlm.local_cli()
    ...
    def from_env() -> ApiLlm:
        ...
        ...
    @staticmethod
    def messages_api(api_key: str, model: str) -> ApiLlm: ...
    @staticmethod
    def messages_api_with_url(api_key: str, model: str, base_url: str) -> ApiLlm: ...
    @staticmethod
    def openai(api_key: str, model: str) -> ApiLlm: ...
    @staticmethod
    def openai_with_url(api_key: str, model: str, base_url: str) -> ApiLlm: ...
    @staticmethod
    def local_cli() -> ApiLlm: ...
...
class PipelineBuilder:
    ...
    def refine(self, validator: Any, max_iter: int = 5, target: float = 1.0) -> PipelineBuilder: ...
    def extract(self, lang: str) -> PipelineBuilder: ...
    def best_of(self, n: int = 3) -> PipelineBuilder: ...
    def ensemble(self, n: int = 3) -> PipelineBuilder: ...
    def reason(self) -> PipelineBuilder: ...
    def map(self, func: Callable[[str], str]) -> PipelineBuilder: ...
    def defaults(self, defaults: Defaults) -> PipelineBuilder: ...
    def skill(self, skill: Skill) -> PipelineBuilder: ...
    def go(self) -> PipelineResult: ...
...
class ConcurrentRunnerBuilder:
    ...
    def task(self, label: str, prompt: str, builder_fn: Callable[[PipelineBuilder], PipelineBuilder]) -> ConcurrentRunnerBuilder: ...
    def simple_task(self, label: str, prompt: str) -> ConcurrentRunnerBuilder: ...
    def max_concurrency(self, n: int) -> ConcurrentRunnerBuilder: ...
    def go(self) -> List[ConcurrentTaskResult]: ...

def pipeline(llm: LlmCallable, prompt: str) -> PipelineBuilder:
    ...
    Returns:
        A builder for composing pipeline steps.
    """
    ...

def concurrent(llm: LlmCallable) -> ConcurrentRunnerBuilder:
    ...
    Returns:
        A builder for adding concurrent tasks.
    """
    ...
...
class Objective:
    ...
    def __init__(self, name: str) -> None: ...
    def weight(self, w: float) -> Objective: ...
    def target(self, t: float) -> Objective: ...
...
class MultiObjectiveBuilder:
    ...
    def scalarize(self, strategy: str) -> MultiObjectiveBuilder: ...
    def objective(self, obj: Objective, validator: Any) -> MultiObjectiveBuilder: ...
    def build(self) -> MultiObjectiveValidator: ...
    def __repr__(self) -> str: ...
...
class MultiObjectiveValidator:
    ...
    def validate(self, text: str) -> ScoreResult: ...
    def validate_multi(self, text: str) -> List[Dict[str, Any]]: ...
    @property
...
class ParetoFront:
    ...
    @property
    def solutions(self) -> List[ParetoCandidate]: ...
    @property
    def is_empty(self) -> bool: ...
    @property
    def best(self) -> Optional[ParetoCandidate]: ...
    def __len__(self) -> int: ...
    def __repr__(self) -> str: ...

class ParetoRefineResult:
    ...
    @property
    def objective_bests(self) -> List[Tuple[str, float]]: ...
    def objective_bests_dict(self) -> Dict[str, float]: ...
    def __repr__(self) -> str: ...

def multi_objective() -> MultiObjectiveBuilder:
...
def refine_pareto(
    ...
    Args:
        llm: A callable (prompt: str, feedback: Optional[str]) -> str
        prompt: The initial prompt.
        validator: A MultiObjectiveValidator.
        max_iter: Maximum iterations (default 10).

    Returns:
        A ParetoRefineResult with the Pareto front and statistics.
    """
    ...
...
class Dataset:
    ...
    def __init__(self) -> None: ...
    def example(self, input: str, expected: str) -> Dataset: ...
    def labeled_example(self, input: str, expected: str, label: str) -> Dataset: ...
    def len(self) -> int: ...
    def is_empty(self) -> bool: ...
    def __len__(self) -> int: ...
    def __repr__(self) -> str: ...
...
class OptimizerBuilder:
    ...
    def dataset(self, dataset: Dataset) -> OptimizerBuilder: ...
    def metric(self, metric: Callable[[str, str], float]) -> OptimizerBuilder: ...
    def validate(self, validator: Any) -> OptimizerBuilder: ...
    def strategy(self, name: str, max_examples: int = 3, num_candidates: int = 5) -> OptimizerBuilder: ...
    def go(self) -> OptimizeResult: ...
    def __repr__(self) -> str: ...

def optimizer(llm: LlmCallable, prompt: str) -> OptimizerBuilder:
    ...
    Returns:
        An OptimizerBuilder for configuring the optimization.
    """
    ...
...
class Metric:
    ...
    Example:
        ...
        m.evaluate("hello", "hello")  # 1.0

        m = Metric.contains()
        m.evaluate("the answer is 42", "42")  # 1.0
    ...
    def f1_token() -> Metric: ...
    @staticmethod
    def custom(name: str, func: Callable[[str, str], float]) -> Metric: ...
    def evaluate(self, prediction: str, expected: str) -> float: ...
    def name(self) -> str: ...
    def __repr__(self) -> str: ...
...
class StepDef:
    ...
    def then(self, next: StepDef) -> StepDef:
        """Sequential: run self, feed output to next."""
        ...
    def race(self, other: StepDef) -> StepDef:
        """Parallel: run both, pick highest score."""
        ...
    def par(self, other: StepDef) -> StepDef:
        """Parallel: run both, concatenate outputs."""
        ...
    def fallback(self, other: StepDef) -> StepDef:
        """If self produces score == 0, try other."""
        ...
    def retry(self, n: int = 3, target: float = 0.9) -> StepDef:
        """Retry up to N times until score >= target."""
        ...
    def map(self, func: Callable[[str], str]) -> StepDef:
        """Transform the output text."""
        ...
    def run(self, input: str) -> StepResult:
        """Execute this step with the given input text."""
        ...
    def __repr__(self) -> str: ...

def step_fn(name: str, func: Callable[[str], str]) -> StepDef:
    """Create a step from a Python callable.

    The callable takes a string and returns a string. Score is always 1.0.

    Args:
        name: Step name for debugging.
        func: A callable (input: str) -> str.

    Returns:
        A StepDef that can be composed with other steps.
    """
    ...

def step_scored(name: str, func: Callable[[str], Tuple[str, float]]) -> StepDef:
    """Create a scored step from a Python callable.

    The callable takes a string and returns (text, score).

    Args:
        name: Step name for debugging.
        func: A callable (input: str) -> (str, float).
...
def run_all_steps(input: str, steps: List[StepDef]) -> List[StepResult]:
    """Run multiple steps concurrently on the same input.

    Args:
        input: The input text.
        steps: List of StepDef instances.

    Returns:
        A list of StepResult, one per step.
    """
    ...
