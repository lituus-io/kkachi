...
impl PyApiLlm {
        ...
        match (
            ...
            (Some(cap), Some(rps), None) => {
                self.built = Some(BoxedLlm::new(
                    base.with_cache(cap).with_rate_limit(rps),
                ));
            }
            (Some(cap), None, Some(retries)) => {
                self.built = Some(BoxedLlm::new(
                    base.with_cache(cap).with_retry(retries),
                ));
            }
            (None, Some(rps), Some(retries)) => {
                self.built = Some(BoxedLlm::new(
                    base.with_rate_limit(rps).with_retry(retries),
                ));
            }
    ...
    pub fn get_llm(&mut self) -> &BoxedLlm<'static> {
        self.ensure_built();
