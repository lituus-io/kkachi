...
/// llm = ApiLlm.messages_api_with_url("key", "model", "https://custom.api.com")
...
impl PyApiLlm {
    ...
    ///
    /// Checks in order:
    /// 1. ANTHROPIC_API_KEY → Messages API provider
    /// 2. OPENAI_API_KEY → OpenAI
    /// 3. CLI binary in PATH → local CLI
    ///
        ...
        ApiLlm::from_env().map(Self::new_plain).map_err(|e| {
            PyRuntimeError::new_err(format!(
                "No LLM provider found. Set ANTHROPIC_API_KEY, OPENAI_API_KEY, \
                 or ensure CLI binary is in PATH. Error: {}",
                e
            ))
        })
    }
    ...
    #[staticmethod]
    fn messages_api(api_key: String, model: String) -> Self {
        Self::new_plain(ApiLlm::messages_api(api_key, model))
    }

    /// Create a Messages API client with custom base URL.
    #[staticmethod]
    fn messages_api_with_url(api_key: String, model: String, base_url: String) -> Self {
        Self::new_plain(ApiLlm::messages_api_with_url(api_key, model, base_url))
    }
    ...
    /// Create a local CLI client.
    #[staticmethod]
    fn local_cli() -> Self {
        Self::new_plain(ApiLlm::local_cli())
    }
