...
pub(crate) struct PyCallableLlm {
    callable: PyObject,
}

impl PyCallableLlm {
    pub(crate) fn new_from_ref(py: Python<'_>, callable: &PyObject) -> Self {
        Self {
...
impl Tool for PyCallableTool {
    type ExecuteFut<'a>
        = std::future::Ready<kkachi::error::Result<String>>
    where
        Self: 'a;
...
impl PyReasonBuilder {
    ...
    fn go(&self) -> PyResult<PyReasonResult> {
        ...
        let rust_skill_text = self
            .skill
            .as_ref()
            .map(|s| s.to_skill().render())
            .filter(|s| !s.is_empty());
        ...
        macro_rules! apply_config {
            ($builder:expr) => {{
                ...
                if let Some(ref defaults) = rust_defaults {
                    b.config.gen.defaults = Some(defaults.clone());
                }
                if let Some(ref skill_text) = rust_skill_text {
                    b.config.gen.skill_text = Some(skill_text.clone());
                }
                b
            }};
        }
