...
#[cfg(feature = "storage")]
use kkachi::recursive::{DuckDBVectorStore, HashEmbedder, VectorStore};
use kkachi::{
    diff::{DiffRenderer, DiffStyle, TextDiff},
    recursive::cli::{CliExecutor, CommandResult},
};
use std::fs;
...
fn parse_rust_errors(result: &CommandResult) -> Vec<String> {
...
/// Example 4: DuckDB RAG with CLI Validation
///
/// Combines DuckDB vector store for few-shot learning with CLI validation.
...
fn example_4_duckdb_rag_with_cli() -> anyhow::Result<()> {
    ...
    println!("Step 1: Seeding DuckDB with {} examples...", examples.len());
    ...
    let results = store.search_text(query, 3);
...
fn main() {}"#;
    ...
    if result.success {
        ...
        // Store successful result back to DuckDB
        println!("\nStep 5: Storing successful code in DuckDB...");
        let new_id = format!("rust:yaml_parse_{:x}", hash_code(generated_code));
        store.add(&new_id, generated_code)?;
        println!("  Stored as: {}", new_id);
        println!("  Total examples in store: {}", store.len());
    } else {
