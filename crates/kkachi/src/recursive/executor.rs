...
/// GAT-based trait for code executors.
///
/// Uses Generic Associated Types for zero-cost async without boxing.
/// For object-safe collections, use [`DynCodeExecutor`] which provides
/// a blanket impl from any `CodeExecutor`.
...
/// Object-safe bridge for [`CodeExecutor`] implementations.
///
/// Any type implementing [`CodeExecutor`] automatically implements
/// `DynCodeExecutor` via blanket impl.
pub trait DynCodeExecutor: Send + Sync {
    ...
    /// Execute the given code (boxed future).
    fn execute_dyn<'a>(
        &'a self,
        code: &'a str,
