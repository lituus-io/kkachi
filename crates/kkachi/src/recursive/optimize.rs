...
/// For new code, prefer using the [`crate::metric::Metric`] trait directly via
/// [`Optimizer::metric_impl`].
...
impl<'a, L: Llm> Optimizer<'a, L> {
    ...
    /// Set a metric using the [`crate::metric::Metric`] trait.
    ///
    /// Bridges the standard Metric trait into the optimizer.
