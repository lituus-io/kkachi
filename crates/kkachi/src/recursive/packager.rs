...
impl<'a> PackagerBuilder<'a> {
    ...
    pub fn build(&self) -> Result<PackageResult> {
        ...
        let wheel_name = format!(
            "{}-{}-py3-none-any.whl",
            normalized_name, self.meta.version
        );
        ...
        write_zip_entry(&mut zip, &init_path, init_py.as_bytes(), options, &mut records)?;
        ...
        write_zip_entry(&mut zip, &metadata_path, metadata.as_bytes(), options, &mut records)?;

        // 4. WHEEL
        let wheel_meta = generate_wheel_metadata();
        let wheel_meta_path = format!("{}/WHEEL", dist_info);
        write_zip_entry(&mut zip, &wheel_meta_path, wheel_meta.as_bytes(), options, &mut records)?;
        ...
        let size_bytes = std::fs::metadata(&wheel_path)
            .map(|m| m.len())
            .unwrap_or(0);
...
mod tests {
    ...
    fn test_wheel_record_sha256() {
        ...
        let mut record_content = String::new();
        {
            ...
            let mut record = archive
                .by_name("hash_test-0.1.0.dist-info/RECORD")
                .unwrap();
    ...
    fn test_mmap_db_integrity() {
        ...
        let file = std::fs::File::open(&result.wheel_path).unwrap();
        let mut archive = zip::ZipArchive::new(file).unwrap();
        let mut db_in_wheel = archive
            .by_name("integrity_test/data/knowledge.db")
            .unwrap();
