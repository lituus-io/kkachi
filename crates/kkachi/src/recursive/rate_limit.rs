        ...
        loop {
            match &mut this.state {
                ...
                RateLimitState::WaitingForSlot(sleep) => match sleep.as_mut().poll(cx) {
                    Poll::Ready(()) => match this.bucket.lock().expect("rate limiter lock poisoned").try_acquire() {
                        None => {
                ...
                RateLimitState::WaitingForSlot(start, duration) => {
                    if start.elapsed() >= *duration {
                        match this.bucket.lock().expect("rate limiter lock poisoned").try_acquire() {
                            None => {
                                let fut =
...
impl<L: Llm> Llm for RateLimitedLlm<L> {
    ...
    fn generate<'a>(
        ...
        match self.state.lock().expect("rate limiter lock poisoned").try_acquire() {
            None => {
