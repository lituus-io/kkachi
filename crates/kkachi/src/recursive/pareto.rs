...
impl<A: Validate, B: Validate, C: Validate> MultiObjective3<A, B, C> {
    fn validate_multi_impl(&self, text: &str) -> MultiScore<'static> {
        ...
        // Inline each validator call to avoid &dyn Validate casts
        macro_rules! score_objective {
            ($obj:expr, $val:expr) => {{
                let score = $val.validate(text);
        ...
        score_objective!(&self.obj_a, &self.val_a);
        score_objective!(&self.obj_b, &self.val_b);
        score_objective!(&self.obj_c, &self.val_c);
...
pub fn refine_pareto_sync<L: crate::recursive::llm::Llm, V: MultiObjectiveValidate>(
    ...
    shared::block_on(refine_pareto(llm, prompt, validator, max_iter))
}
