...
mod tests {
    ...
    async fn test_llm_critic_basic() {
        ...
        let feedback = critic
            .critique("Write a Rust function", "fn add(a: i32, b: i32) -> i32 { a + b }", score)
            .await
            .unwrap();
        ...
        assert!(feedback.has_suggestions());
        assert!(feedback.suggestions_str().unwrap().contains("documentation"));
    }

    #[tokio::test]
    async fn test_llm_critic_no_suggestions_marker() {
        let llm = MockLlm::new(|_, _| {
            "ANALYSIS: The output is perfect. No issues found.".to_string()
        });
    ...
    fn test_llm_critic_parse_response_structured() {
        ...
        assert_eq!(suggestions, Some("Add more tests and documentation.".to_string()));
    }
    ...
    async fn test_fn_critic_with_suggestions() {
        let critic = FnCritic::new(|_prompt, output, _score_val| {
            ...
            if has_fn {
                ...
                CriticFeedback::new(Score::fail("No function"), Cow::Borrowed("Missing function"))
    ...
    async fn use_critic_generic<'a, C: Critic>(
        ...
        critic
            .critique("test prompt", output, score)
            .await
            .unwrap()
    }
