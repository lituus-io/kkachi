...
impl TypedSignature {
    ...
    pub const fn input(mut self, name: Sym, kind: ValueKind) -> Self {
        ...
        self.inputs[self.input_count as usize] =
            TypedField::new(name, kind, Direction::Input);
        self.input_count += 1;
        self
    }
    ...
    pub const fn output(mut self, name: Sym, kind: ValueKind) -> Self {
        ...
        self.outputs[self.output_count as usize] =
            TypedField::new(name, kind, Direction::Output);
        self.output_count += 1;
        self
    }
...
impl<'a> ParsedOutput<'a> {
    ...
    pub fn get_bool(&self, name: Sym) -> Option<bool> {
        self.get_str(name).and_then(|s| match s.to_ascii_lowercase().as_str() {
...
mod tests {
    ...
    fn validator_missing_required_field() {
        ...
        assert!(score.feedback_str().unwrap().contains("Missing required field"));
    }

    #[test]
    fn validator_wrong_type() {
        const SIG: TypedSignature = TypedSignature::new("Count things.")
            .output(SYM_SCORE, ValueKind::Int);
    ...
    fn validator_no_outputs() {
        const SIG: TypedSignature = TypedSignature::new("Input only")
            .input(QUESTION, ValueKind::Str);
