...
impl<'a> ModuleState<'a> {
    ...
    pub fn child_mut(&mut self, name: &str) -> Option<&mut ModuleState<'a>> {
        self.children.iter_mut().find(|c| c.name == name).map(|c| c.as_mut())
    }
...
/// ```
pub struct PredictModule<'a, L: crate::recursive::Llm, V: crate::recursive::Validate = crate::recursive::NoValidation> {
    name: &'a str,
    llm: &'a L,
    instruction: String,
    validator: V,
...
impl<'a, L: crate::recursive::Llm, V: crate::recursive::Validate> PredictModule<'a, L, V> {
    ...
    pub fn validate<V2: crate::recursive::Validate>(self, validator: V2) -> PredictModule<'a, L, V2> {
        PredictModule {
            name: self.name,
            llm: self.llm,
    ...
    pub fn save_composable_state(&self) -> ModuleState<'_> {
        let mut state = ModuleState::new(self.name)
            .with_instructions(Cow::Borrowed(self.instruction.as_str()));
    ...
    V: crate::recursive::Validate + 'static,
{
    ...
    type ForwardFut<'b> = std::pin::Pin<Box<dyn Future<Output = crate::error::Result<String>> + Send + 'b>>
    where
        Self: 'b;
...
mod tests {
    ...
    fn test_module_state_child_mut() {
        let mut state = ModuleState::new("root")
            .with_child(ModuleState::new("a"));
    ...
    fn test_module_state_json_roundtrip() {
        let state = ModuleState::new("predict")
            ...
            .with_demo("cat", "chat")
            .with_child(
                ModuleState::new("retrieve")
                    .with_instructions("Search for relevant documents."),
            );
    ...
    fn test_module_state_nested_json() {
        let state = ModuleState::new("root")
            .with_child(
                ModuleState::new("l1")
                    .with_child(ModuleState::new("l2").with_instructions("deep")),
            );
    ...
    fn test_macro_save_state() {
        ...
        assert_eq!(
            state.children[0].instructions.as_deref(),
            Some("Find docs")
        );
    ...
    fn test_predict_module_save_load_state() {
        let llm = crate::recursive::MockLlm::new(|_, _| "mock".to_string());
        let module = PredictModule::new("qa", &llm, "Original instruction")
            .demo("in1", "out1");
    ...
    fn test_predict_module_load_wrong_name() {
        ...
        let state = ModuleState::new("wrong_name")
            .with_instructions("should not apply");
