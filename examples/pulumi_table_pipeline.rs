...
//
// Rewritten using kkachi v0.1.0 composable orchestration APIs:
...
// Requires:
//   - LLM_API_KEY_PATH environment variable (or edit the constant below)
...
fn config_source_folder() -> String {
    std::env::var("SOURCE_FOLDER").unwrap_or_else(|_| {
        "/path/to/pulumi/templates".to_string()
    })
}

fn config_llm_api_key_path() -> String {
    std::env::var("LLM_API_KEY_PATH")
        .unwrap_or_else(|_| "/path/to/llm/api-key".to_string())
}

fn config_gcp_creds() -> String {
    std::env::var("GCP_CREDS").unwrap_or_else(|_| {
        "/path/to/gcp/credentials.json".to_string()
    })
}
...
// =============================================================================
// Main Pipeline — Composable Orchestration (v0.1.0)
// =============================================================================

#[cfg(feature = "api")]
fn main() -> anyhow::Result<()> {
    println!("{}", "=".repeat(70));
    println!("Kkachi Pulumi Table Pipeline (Rust v0.1.0 — Composable Orchestration)");
    println!("{}\n", "=".repeat(70));
    ...
    let api_key = fs::read_to_string(config_llm_api_key_path())
        .expect("Failed to read API key file")
        .trim()
        .to_string();
    let llm = ApiLlm::openai_with_url(&api_key, "your-model-id", "https://your-api-endpoint.com")
        .timeout(120)
        .with_retry(3);
...
A ```yaml code block with category, subcategory, template_name: {folder_name}, keywords, gcp_services, pulumi_resources"#,
    ...
    println!("{}", "=".repeat(70));
    println!("Pipeline complete! (v0.1.0 Composable Orchestration)");
    ...
    println!();
    println!("  New v0.1.0 features used:");
